    ...
    ListView {
        ...
        delegate: Rectangle {
            ...
            MouseArea{
                ...
                onClicked: {
                    if( item.directory )
                        document_dialog.open(item.path)
                    else {
                        var id = repository.insert( item.path )
                        document_dialog.selected(id)
        ...
        Component.onCompleted: {
            ...
            refresh( kaqaz.documentsLocation() )
        }
        ...
        function refresh( path ){
            ...
            var files = kaqaz.dirEntryFiles(path,["*.pdf","*.txt","*.text"])
